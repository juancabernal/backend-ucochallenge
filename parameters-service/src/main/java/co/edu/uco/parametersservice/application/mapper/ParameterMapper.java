...
public class ParameterMapper {
    ...
    public Parameter toDomain(ParameterEntity entity) {
        return new Parameter(entity.getId(), entity.getKey(), entity.getValue(), entity.getUpdatedAt());
    }
    ...
    public Parameter from(String key, String value) {
        return new Parameter(null, key, value, Instant.now());
    }

    public Parameter withUpdatedValue(Parameter original, String value) {
        return new Parameter(original.id(), original.key(), value, Instant.now());
    }

    public Parameter withGeneratedId(Parameter parameter) {
        UUID id = parameter.id() != null ? parameter.id() : UUID.randomUUID();
        return new Parameter(id, parameter.key(), parameter.value(), parameter.updatedAt());
    }
}
